...
- 使用官方慢速步态 `motion_id=303`、真机 ABI 定义的视觉来源 `cmd_source=2`；第一赛段候选前进限幅为 0.40 m/s，横移限幅 0.08 m/s、转向限幅 0.30 rad/s。第一赛段非零运动使用 0.15 m 步高，其余赛段使用 0.05 m；赛段信息缺失时回退到 0.05 m。START、END 和静止 DATA 固定发送零步高，避免零速度时仍原地踏步。0.40 m/s 与 0.15 m 步高是逐级实测组合，不代表官方 303 预设已验证该包络。
...
第一赛段比赛控制器还会把进入 RUNNING 时的 odom 位姿锁定为中心线：融合航向与横向误差回拉，
横偏超过 0.08 m 开始减速，达到 0.18 m 时输出归零并报告 `TRACK_DEVIATION`。该保护防止持续漂移，
但不能替代黄色边界识别、石板物理事实和现场赛道标定。
六个赛段的 RUNNING 输出还经过全局外边界保护：标定后的 `course_pose` 必须位于 4×16 m 场地内，
最终不可侵入余量为 0.08 m；控制器从 0.30 m 开始减速回拉，并按 0.20 s 反应时间和
0.40 m/s² 限减速度预测停车点。当前位置或预测停车点触及余量、以及坐标缺失/非法时均输出零速。
该保护依赖比赛日现场坐标变换标定，在 `site_transform_valid=false` 时保持 fail-closed，不能绕过。
