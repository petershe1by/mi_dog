...
改变，再发送 CONTINUE。默认 UI 为正式比赛模式，后端直接拒绝人工移动和姿态。只有赛前维护
且具备防护工装时才允许用 `--maintenance-controls` 重新启动；正式比赛不得携带此参数。即使
maintenance 当前为 `enable_motion=true`，非零脉冲仍必须同时取得实时 `run_allowed=true`；
任一门不满足都应拒绝。
...
继续因相机缺失而 fail-closed。2026-08-19 已完成 ARM64 假接口和前三真机周期无运动验收；
比赛预检仍必须在活动窗口取得真实帧，不能以 guard 进程存在代替帧证据。
