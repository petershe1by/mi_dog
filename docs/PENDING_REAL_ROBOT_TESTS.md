# 待完成真机测试清单

更新日期：2026-08-21（Asia/Shanghai）

本文只记录尚未完成的真机验证和实现，勾选前必须保留日志、版本、配置、视频或物理结果证据。
Gazebo 六赛段已经完成，但不能作为下面任何真机项目的替代证据。

## 比赛冲刺顺序

时间紧张，按以下顺序推进，不重复已经完成的超声、ToF、基础运动和停止链测试：

1. [已完成] 电量充足、拔除充电线后，UI `111` 起立和 `101` 安全趴下均已取得真机完成反馈。
2. [已完成] 把官方赛题 PDF 第 3 页平面图转换为真机课程参数和坐标框架。
3. [框架已完成] 六赛段任务事实门、定位置信度、检查点、失败闭锁和 JSONL 回放已离线回归；
   继续实现并校准真实视觉生产者，保持 `course_calibrated=false`。
4. 在实际赛道或等比例关键设施上逐段验收，然后完成一次全程自主彩排。
5. 最后设置 `course_calibrated=true`、切换 competition、通过只读预检并冻结最终版本。

相机逐帧延迟、自然放电低电量门和额外故障注入属于非阻塞增强项；比赛主线完成前不占用真机时间。

## 官方场地参数已取得

- [x] 官方赛题 PDF 第 3 页包含完整平面图；正文明确全场 `400 cm × 1600 cm`、大部分净宽
  `100 cm`。已读取起点、4×4 球阵、曲道、三通道、独木桥和终点区的尺寸标注，不再把
  “寻找场地尺寸”列为阻塞项。
- [x] 已确认固定物尺寸：石板 `100×30×5 cm`、间隔 `20 cm`；球直径 `20 cm`；限高杆
  `110 cm`、截面 `10×10 cm`、底部离地 `40 cm`；障碍为两个 `20 cm` 方块、间隔
  `20 cm`；球框 `50×30 cm`、深 `50 cm`。
- [x] 已将 40 项平面图固定参数固化进真机课程配置，并增加严格离线校验和坐标变换说明。官方
  正文写黄色边沿 `15 cm`（弯道 `10 cm`），图例
  写黄色边框 `10 cm`。配置保留 `15 cm` 作为官方几何冲突记录；按用户现场选择，实际全局
  外边界保护使用独立 `0.08 m` 最终不可侵入余量，并从 `0.30 m` 开始减速回拉、预测制动点。

## 当前禁止条件

- 最近一次只读状态为 maintenance、`DOWN_WAITING/run_allowed=false`；充电时运控会报告
  `motion_switch_status=14`，这是预期的充电闭锁，不是可运动状态。动作测试前必须达到清单要求、
  拔除 `charge` 线并重新检查 BMS 和运控已恢复正常，不依赖旧电量或旧状态。
- 维护/比赛适配器配置为 `enable_motion=true`，但任何真实输出同时受 Supervisor、传感器、
  watchdog、倾角、雷达和课程标定门约束；不得直接发布运动话题或绕过这些安全门。
- 真机比赛控制器默认 `course_calibrated=false`。在课程参数和六赛段物理结果未验证前，即使
  Supervisor 收到 START，控制器也必须保持零输出并报告 `COURSE_UNCALIBRATED`。
- 真实运控测试时必须拔除充电线，并使用四脚离地架、防滑空场或厂家认可的防护工装。

## A. 已实现，只缺物理验收

- [x] 历史版本在真实 `motion_servo_cmd` 上确认静止 `safe_cmd_vel` 只保持零速/零步高 END，不发送
  START/DATA、不进入 `motion_id=303`、四足接触不重置且无位移。原“用零速度
  START→DATA→END 验证停止链”的假设已被真机否定：303 模式即使速度和步高为零仍会切换重心，
  因而不能作为无动作测试。真实 END-only 验收取得 8 个 END、无 START/DATA，运控始终为
  `motion_id=112`，90 组四足接触均为 0.5，odom XY 变化 0.000018 m，最终 PAUSED。证据见
  `docs/evidence/2026-08-13_real_zero_servo_attempt.txt`。
- [x] 2026-08-17 修复空闲重复 END 导致原厂 `BAN_TRANS=5/3022`：无活动 Servo 会话时完全
  静默，活动会话退出只发送一次可靠 END。2026-08-18 新版真实活动会话取得严格
  `START=1, DATA=1, END=1`，END 后无 DATA/重复 END，STOP 后 6 秒零帧且无 BAN_TRANS/3022；
  用户确认短距前进和停车正常。证据见 `docs/evidence/2026-08-18_single_end_real_acceptance.txt`。
- [x] 在一次 `0.05 m/s × 0.25 s` 前进 canary 中验证电脑 PAUSE 撤销 `run_allowed`、在
  0.50 秒内发送 END；odom XY 变化 0.031935 m，用户确认短距前进和停车正常。该结果只覆盖
  一次前进 canary，不替代 watchdog、活动重启或六向验收。证据见
  `docs/evidence/2026-08-15_bounded_forward_canary.txt`。
- [x] 在 `0.05 m/s` 有界低速非零会话中验证结构化 STOP 在 0.50 秒内 END，随后限权重启
  `mi-dog-real-sensor.service`；重启后保持 `DOWN_WAITING/run_allowed=false`、正式
  `enable_motion=False`，继续测试前进命令只有 END。odom XY 0.014769 m，用户确认运动、停车、
  重启和最终趴卧均正常。证据见
  `docs/evidence/2026-08-15_active_stop_restart_acceptance.txt`。
- [x] 在有界低速非零会话中验证 0.30 秒命令 watchdog。修正 odom 基线后的复测取得
  XY 0.034104 m、Z -0.001086 m，watchdog END 在 0.50 秒内；用户确认正常前进、及时停车并
  始终站立。证据见 `docs/evidence/2026-08-15_bounded_watchdog_attempt.txt`。
- [x] 在隔离许可话题的有界低速非零会话中验证 supervisor 0.50 秒许可过期：失鲜后 0.60 秒
  内 END，END 后持续前进命令无 DATA，用户确认正常停车；实测 XY 0.050199 m、Z
  -0.000596 m。工具因严格 `<0.05 m` 断言输出 FAIL，但超限 0.199 mm 小于既有静止 odom
  0.784 mm 漂移；不改写工具结果，按时序、物理观察和最终闭锁独立验收功能。证据见
  `docs/evidence/2026-08-15_permission_timeout_acceptance.txt`。
- [x] 断开电脑链路后狗内置主控保持原 MainPID、`NRestarts=0`，Supervisor 继续锁存
  `EMERGENCY_STOP/run_allowed=false`；重新连接后的只读审计取得相机 70、雷达 71、odom 326、
  Servo 0 帧且无 BAN_TRANS/3022。证据见
  `docs/evidence/2026-08-18_laptop_disconnect_acceptance.txt`。
- [x] 电量充足、拔除充电线后，逐个验证六向 0.25 秒低速调试脉冲和 STOP。六次后端命令均
  `run_allowed=true`，日志严格六次会话开始/六次单次 END，最终 STOP 后 6 秒 Servo 0 帧，
  无 BAN_TRANS/3022；用户确认左右转映射正确。该轮未采集逐向里程计/视频位移，不扩大为精确
  停止距离证据。见 `docs/evidence/2026-08-18_six_direction_jog_acceptance.txt`。
- [x] 2026-08-19 电量 99%、拔除充电线并清空周边后，通过 UI 同一后端验证原厂 `111` 起立和
  `101` 安全趴下；测试后保持 maintenance 并确认运动许可重新闭锁；
  111/101 分别取得 `result=true/code=0`、进度 100%/99% 和完成反馈；最终 maintenance、
  `DOWN_WAITING/run_allowed=false`、空闲 Servo 0 帧。证据见
  `docs/evidence/2026-08-19_posture_arm64_acceptance.txt`。
- [x] 通过 UI 开启头部 RGB SSH-MJPEG 连续运行 2 分钟：709 帧，6.03–6.83 fps，约
  1.26 Mbit/s；断开后远端转码进程立即回收，30 秒重连取得 131 帧且无读取错误；同时记录了
  转码/相机/正式节点 CPU、内存和 Tegra 温度。证据见
  `docs/evidence/2026-08-13_ui_rgb_acceptance.txt`。
- [x] 2026-08-19 完成 100 帧原生相机与实际 UI SSH-JPEG 链路测试：相机时间戳到 ROS 回调
  平均 `26.267 ms`、P95 `47.664 ms`；UI 首帧启动 `8.622 s`，稳定传输约 `4.24 fps`、
  `0.500 Mbit/s`。首帧启动时间没有冒充逐帧显示延迟；图像仅在内存处理且未保存。证据见
  `docs/evidence/2026-08-19_camera_latency_acceptance.txt`。
- [x] 完整重启验证延迟 RGB 启动：安全节点应立即进入 `DOWN_WAITING`，uptime 至少 240 秒后
  才请求 command 9，必须取得真实图像帧，并连续稳定至少 3 分钟；服务返回 result 0 不能替代帧证据。
  2026-08-18 两次完整重启均按门限取得 640×480 实帧；正式 180 秒复验为 1792 帧、
  10.012 fps、最长帧间隔 0.310 秒，且无 NvCapture/Capture Scheduler 错误。
- [x] 2026-08-19 新启动周期暴露原厂连续采集仍可复现资源耗尽：约 67 秒后出现
  `Free request list is empty` 并停止。重启 Argus/bringup 后，显式 STOP→START 的两轮分别
  连续发布约 63 秒和 81 秒，第二轮 20 秒探针取得 201 个有效 640×480 `bgr8` 帧，本轮没有
  新 NvCapture/Argus 错误。45 秒活动、5 秒冷却、实帧巡检、失败单次 STOP 的 guard 已通过
  离线成功/丢帧故障注入、ARM64 假接口和真机前三周期验收；三轮均有新的实帧序列、单次
  START/STOP 且无新相机错误。审计修正版又在 17:17:55、17:18:49、17:19:42 开始三轮，
  分别在 45 秒后停止；最终 `DOWN_WAITING/run_allowed=false`、Servo 0 帧。
- [ ] （非阻塞增强）运行中模拟低于 30% 的真实 BMS 条件不可人为制造；在自然放电接近阈值时验证锁存 PAUSED，
  条件恢复不自动继续。

## B. 感知状态与比赛主线

- [x] 修复前向激光空场机身/地面回波误停：使用自回波包络、连续角度障碍簇、三帧确认和
  前超声融合；ARM64 纯算法与 ROS 构建通过。空场只读 143 个融合样本为 0.413..0.516 m，
  超声为 0.561..0.655 m，`DOWN_WAITING/run_allowed=false` 且空闲 Servo 0 帧。历史 0.3 m
  纸箱超声证据与合成障碍簇回归已覆盖基本停车门，修改后真实纸箱闭锁保留为赛前抽检。

- [x] 超声避障验收完成；用户已确认无需重复不同材质、宽度和偏置测试，比赛前不再扩测。
- [x] 头部 ToF 地面/落差用途和真机响应已验收；用户已确认无需重复测试，比赛前不再扩测。
- [x] 已实现真实 `course_observation` 生产者骨架：直接解析 640×480 BGR/RGB、黄色双边界、橙色
  候选、前向雷达和 odom 四元数，发布严格 schema、传感器鲜度、站点变换质量和检测摘要；默认
  `site_transform_valid=false`、`localization_confidence=0`、`facts={}`，不会用颜色或里程伪造
  赛段完成。离线合成、失鲜、odom 跳变、ARM64 ROS 隔离均通过。
- [ ] 相机目标识别实物数据：石板、橙球、道路边界、隧道、桥、足球、球门和终点圈；当前只有
  黄色/橙色候选和接口，尚无这些对象的现场阈值、误检率或完成事实证据。
- [ ] 真机定位现场标定：odom 到赛道坐标的变换和跳变拒绝已实现；仍需测量起点原点/航向、
  漂移和跌落放回后的重定位，才能设置 `site_transform_valid=true`。
- [x] 无场地异常闭锁：严格 observation schema、事实字段白名单和布尔/计数类型；相机失鲜、
  定位未标定、odom 跳变、未知事实及默认课程未标定均保持零输出。真实遮挡阈值仍随实物验收。
- [x] 六赛段外边界输出门：标定后的 `course_pose` 检查 4×16 m 外框，最终余量 0.08 m；按
  0.20 s 反应时间和 0.40 m/s² 限减速度预测停车点，坐标缺失/非法、当前位置或预测点触线均
  零输出。2026-08-21 ARM64 构建、纯算法及 `/mi_dog_test` 隔离 ROS 验收通过；尚缺现场坐标
  标定和有防护的低速物理边界验收。

## C. 功能尚未实现，之后才能测试

当前 `race_controller.py` 是带课程标定闭锁的通用低速/雷达避障骨架，不是六赛段完成版。
默认参数只是占位值；预检会在 `course_calibrated=false` 时明确失败，不能把固定距离测试当成
石板、触球、过桥或进球证据。

2026-08-19 已移除“累计占位距离即可完成赛段”的正式路径：新任务核心只接受统一感知事实，
覆盖六关完成条件、定位置信度、0.6 秒观测超时、障碍闭锁、检查点快照和离线 JSONL 回放。
尚缺的是产生这些事实的真实视觉/定位节点和物理验收，不能把抽象合成测试写成赛段完成。

- [ ] 真机赛段 1：四块石板顺序、合法出口和边界控制。
- [ ] 真机赛段 2：橙球识别、去重触碰和路径规划。
- [ ] 真机赛段 3：曲道跟踪与实线边界保护。
- [ ] 真机赛段 4：隧道低姿态、碰撞保护和足球入门。
- [ ] 真机赛段 5：桥面对准、全足登桥、越线和跌落恢复。
- [ ] 真机赛段 6：足球越界、四足入圈、稳定停止和安全趴下。
- [ ] 自动停稳、地面/空间判断和安全趴下执行器闭环。
- [ ] 单次真机六赛段全程：不人工发送方向/速度，规定时间内完成并保存日志与视频。

## D. 赛事流程仍需确认或演练

UI 技术入口已完成离线全流程回归：status、PAUSE、STOP、restart、选关和 CONTINUE 均经同一
后端，比赛模式人工方向/姿态继续拒绝，STOP 可绕过写锁。下列项目只剩裁判规则或现场演练，
不是缺少 UI 指令实现。

- [x] 暂停、重启及其现场判罚不由自主代码猜测；操作员按裁判当场口径通过 UI 下发。
- [x] 跌落/出线后的放回位置和恢复赛段由操作员确认后通过 UI 选关/继续。
- [x] 重启后的 START/CONTINUE 选择由操作员通过 UI 给出；服务本身只恢复到闭锁状态。
- [x] 终点判定和允许趴下时机由操作员按裁判口径通过 UI 给出，自动任务不得自行放宽条件。
- [ ] 比赛场地完整彩排：离线启动、电脑暂停/重启、日志导出和备用方案。

## 每次测试必须记录

- Git commit、部署清单路径和 SHA256；
- 电量、温度、是否插充电线、场地和防护方式；
- 输入话题、参数、测试步骤、预期结果和实际结果；
- 是否连接真实运动话题、停止方式、最终姿态和异常；
- 原始日志、视频、数据文件路径和回滚方法。
