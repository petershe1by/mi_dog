...
| 本地比赛 UI | localhost+随机令牌；状态/BMS、START/PAUSE/STOP、赛段继续、移动/姿态闭锁和按需视频 | 原功能 API 真机通过；新增姿态/视频离线回归通过、实机待测 |
...
| UI 远程 RGB | SSH 单连接、JPEG/MJPEG、10 fps 上限及本地实时计数 | 已实现；真实链路帧率/延迟待测 |
...
| UI 人工安全趴下 | 原厂动作 `101`；状态、BMS、运控和 `safe_to_lie_down` 二次门 | 已实现；UI 按钮实机待测 |
