...
| 独立急停 | 软件链完成 | 守卫与 USB HID 常闭输入均通过 ARM64 隔离测试；实体按钮/编码器仍未接入和验收 |
...
- 增加专用 USB HID 输入节点：要求稳定 `/dev/input/by-id` 路径、独占设备和常闭触点；按钮
  按下、触点线断开、USB 拔出或设备打不开都发布急停。正式服务在设备缺失时保持闭锁。
...
1. 急停软件链已完成，但独立实体蘑菇按钮/HID 编码器、真实插拔和实测停止延迟尚未验收。
