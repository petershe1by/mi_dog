...
| 真机传感器桥接 | 已完成基础链 | 相机 guard、scan、odom、BMS、运动状态和 ToF/超声均已真机验收；六关对象事实与站点标定尚缺 |
| 比赛操作 | 基础操作完成 | 启动/暂停/STOP/赛段继续和限权一键重启已真机复验；语音保持闭锁 |
| 比赛 UI/SSH | 已完成基础链 | 姿态、六向低速维护控制和 SSH 视频已真机验证；比赛模式禁人工运动、写互斥、STOP 旁路与裁判事件流离线回归通过 |
...
| 自动安全趴下 | 未完成 | UI 人工 101 已真机通过；`lie_down_request` 尚未连接赛段 6 自动执行闭环 |
| 真机整场比赛 | 未开始 | 基础停止链已验收；六关真实事实感知、现场定位、自动趴下和逐段物理验收尚缺 |

当前服务为 maintenance：`mi-dog-real-sensor.service=active`、`manage_dialogue=false`、
`enable_motion=true`、Supervisor 为 `DOWN_WAITING/run_allowed=false`，没有自主控制器。2026-08-19
最终只读审计 8 秒内真实 Servo 话题零帧，相机/雷达/odom 分别为 50/44/281 帧；当时有线充电，
电量 83%、运控状态 14，闭锁符合预期。控制器的 `course_calibrated=false` 默认闭锁和比赛预检
会继续拒绝未标定课程；sensor-only 仍是 `enable_motion=false` 的回滚路径。
...
2026-08-13 复盘后加固本机 UI：默认比赛模式从后端拒绝人工移动/姿态；非 STOP 写操作共用互斥
锁，STOP 请求可绕过锁派发；姿态 SOC 参数必须为 30..100 的整数；视频改用 15 秒、单次消费的
短期令牌并脱敏日志、持续排空 SSH stderr。localhost 假机器人/假相机回归覆盖 403、409、STOP
请求并发派发、令牌复用拒绝和大 stderr，不代表新增姿态、视频或物理停止已经通过真机验收。
...
1. 真实观测生产者目前只输出黄色/橙色候选、前向净空和 odom 站点变换；石板、球、隧道、
   桥、球门、终点圈以及六关完成事实尚未实现和现场验收，当前固定 `facts={}`。
2. 仍需在实际赛道测量起点原点/航向、随机物体位置和跌落放回重定位，才能设置
   `site_transform_valid=true`。
3. 六个真机赛段必须按 1→6 逐段完成物理验收，并最终完成一次无人方向干预的整场彩排。
4. 自动停稳、地面/空间判断和赛段 6 的安全趴下执行器尚未闭环；UI 人工 101 通过不等于自动闭环。
5. 暂停、重启、跌落放回和终点判定由操作员按裁判口径通过 UI 下发；仍需在正式场地完成流程彩排。
6. 自然放电低于 30% 的真 BMS 复验和更多相机延迟统计是非阻塞增强项，不应挤占比赛主线。
