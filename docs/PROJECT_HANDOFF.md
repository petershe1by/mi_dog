...
| 比赛操作 | 除 UI 一键重启授权外完成 | 启动/暂停/STOP/赛段继续和交互式重启已真机复验；语音保持闭锁 |
| 比赛 UI/SSH | 除一键重启授权外通过 | localhost UI、随机令牌、专用密钥、XTerminal、赛段选择和移动闭锁已真机验证 |
...
检查点恢复及 `enable_motion=False` 移动拒绝均通过。一键 UI 重启只差用户明确批准严格限定的
NOPASSWD systemctl 规则；未批准时会快速失败，不会挂起或改变服务。
...
1. UI 一键重启尚需批准仅允许重启 `mi-dog-real-sensor.service` 的免密 sudo 规则；其他 UI/SSH
   操作已真机复验。
2. 官方 `SERVO_START/DATA/END` 零速度时序及暂停/重启/watchdog 撤权尚无重复验收记录。
...
8. 暂停/重启后的复位位置、计分和当前赛段重试边界仍需技术会议记录。
