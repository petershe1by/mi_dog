...
| 独立急停 | 守卫完成、接口待定 | 内核确认 OTG/device 能力，但 CyberDog 2 三口逐一物理映射仍缺证据；正式服务不启动 HID |
...
- 按用户“参考铁蛋一”的要求只读盘点 Type-C/USB：内核报告 USB2 port 0 `OTG_CAP`、
  ...
  0、`usb0` 无 carrier。这证明至少存在 OTG/device 能力，但不能把能力映射到三个外部物理口。
...
1. 急停守卫已完成，但实体接口方案尚未选定；Type-C 暂参考铁蛋一，内核只读证据确认
   OTG/device 能力，仍需确认 CyberDog 2 三个物理口的逐一映射或采用认可的本体方案。
