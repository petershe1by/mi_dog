...
| 单狗离线启动 | 软件基础完成 | systemd 开机自启、DDS 仅 localhost；仍需拔网线冷启动实测 |
| 真机传感器桥接 | 部分完成 | 里程计、运控、BMS、足端、超声和 ToF 已接入；相机/定位尚未形成比赛感知 |
| 语音/触摸 | 已完成基础事件 | `铁蛋铁蛋` 后使用 `启动/恢复/暂停/终止`；头部双击产生暂停事件 |
...
2026-08-10 最后一次正式服务检查：`mi-dog-real-sensor.service=active`，supervisor 为
`DOWN_WAITING`，`run_allowed=false`，机器狗保持趴卧；没有发送真实运动测试命令。
...
2. 必须完成一次拔网线冷启动，验证 systemd、语音、触摸、传感器和 supervisor 全部仅靠本机。
3. 官方 `SERVO_START/DATA/END` 零速度时序尚未形成可重复验收记录。
4. 超声只完成纸箱静态标定，不同材质、宽度、偏置和动态响应未测。
5. 头部 ToF 已确认看地面，但真实几何落差必须在防坠工装上标定。
6. 相机和定位尚未替代 Gazebo 的 `/model_states` 与世界坐标。
7. 六个真机赛段控制器均未逐段实现和验收。
8. 自动停稳、地面判断和安全趴下执行器尚未闭环。
9. 正式比赛允许的初次语音启动、暂停、触摸和重试范围仍需裁判确认。
