...
更新日期：2026-08-13。单位默认使用 SI。这里汇总当前对话和仓库中保留下来的全部有效数据；
...
### 2026-08-13 头部 RGB UI 真机验收

- 条件：本地代码 `492566e`，默认比赛模式，真机正式服务 active，`enable_motion=False`，
  `DOWN_WAITING/run_allowed=false`；BMS 99%、24.681 V、37°C、有线充电，因此没有发送任何
  姿态、零速度或非零运动命令。
- localhost UI 的 SSH-MJPEG 连续运行 120 秒，取得 709 帧；5 秒滑窗样本为 6.03–6.83 fps，
  最终 6.42 fps，平均约 1.26 Mbit/s、17.53 MB，客户端读取 17,563,648 字节且无错误。
- 流期间转码进程约 19.6–35.9% CPU、128.8–131.7 MB RSS；`camera_server` 约
  33.7–34.9% CPU、202.3–206.9 MB RSS；系统已用内存约 2712–2754 MB、可用约
  4802–4853 MB。正式 real/supervisor 节点约 15.2–15.4%/12.9–13.3% CPU。主控刚启动后
  load average 仍偏高，不能把该区间当成稳定空闲基线。
- 流后 Tegra 样本为 CPU 51.5°C、thermal 48.8–48.95°C、PMIC 50°C。30 秒重连窗口取得
  131 帧、6.02–6.60 fps、约 1.25 Mbit/s；估算首次出帧启动约 8.91 秒。客户端断开后立即
  `pgrep` 未发现转码进程。
- 正在运行的默认 UI 实测 `maintenance_controls=false`，前进和起立 API 均返回 HTTP 403；
  视频令牌日志保持 `<redacted>`。只读审计最终 `PASS`，四个正式节点均单实例。
- 尚未测量相机到浏览器的真实逐帧端到端延迟，也没有保存同一窗口内的自治循环/话题频率；启动
  时间不能当作画面延迟。完整摘要见 `docs/evidence/2026-08-13_ui_rgb_acceptance.txt`。
...
日期：2026-08-10。系统启动 `16:29:29`，systemd 服务在 `16:29:36` active；内核在
`16:29:38` 记录 `eth0: Link is Up`。四个 ROS 节点于 `16:29:51` 启动，`16:29:52`
进入 `DOWN_WAITING`。本机唤醒事件在 `16:33:38` 到达，“暂停”在 `16:33:42` 被接受并使
supervisor 进入 `PAUSED`，离线提示音于 `16:33:45` 完成；期间持续记录 `no motion output`。
...
## 2026-08-13 真实零速度伺服失败与修正

- 拔除充电线后，BMS 95%、`power_normal=true`，现场确认空场安全。正式节点继续保持
  `enable_motion=False`，另启有界临时适配器连接真实 `motion_servo_cmd`。
- 最完整一轮实际取得 START→零速度 DATA→watchdog END，以及重新放行后的
  START→零速度 DATA→PAUSE END；所有 `vel_des` 为零，两个 END 都在 0.50 秒内被捕获。
- 但原实现对 START、DATA、END 均携带 `step_height=[0.05,0.05]`。现场观察到原地踏步和少量
  位移，odom XY 改变量为 0.040369 m，因此该轮明确判定失败，不能用“速度为零”冒充无运动。
- 机器狗本地原厂 303 预设同样使用 0.05 m 步高，而 OFF 预设使用零步高。适配器已改为只有
  实际平移/转向输出非零时才携带配置步高；START、END、静止 DATA 固定零步高。
